...
In this paper, we propose a positivity-preserving algorithm for arbitrary high-order implicit finite volume methods on unstructured grids, to perform robust simulations of complex compressible flows. The basic idea, detailed methodology, and implementation will be presented in the rest of this section.
...
% In this work, we design an algorithm to preserve positivity of cell averages for implicit finite volume methods. In implicit finite volume methods, the future state is obtained through iterations. For instance, in each of the final five stages of the ESDIRK4 time integration presented in Section \ref{ssec:TimeMarching}, inner iterations are performed to obtain the solution to the  nonlinear equation system. Solution at the next time level $\uu^{n+1}$ is the converged state of the last stage. Therefore, to obtain admissible cell averages, we need to ensure that:
% \begin{enumerate}[label=(\alph*)]
%     \item the future state $\uu^{n+1} \in G$, which is the solution to the nonlinear equation \eqref{eq:ESDIRK4}, is admissible,
%     \item the intermediate states $\uu^{\left(s,m\right)} \in G$, which are the iterative solutions to the linearized stage equation \eqref{eq:linearTauUpdate}, are admissible,
% \end{enumerate}
% % \begin{enumerate}[label=(\alph*)]
...
% % \end{enumerate}
% where $\uu^{\left(s,m\right)}$ is the intermediate solution at the $m$-th inner iteration of stage $s$. The future state should be guaranteed to be admissible to make sure that the final converged state exists; the intermediate states should be guaranteed to be admissible to make sure that the solution process can reach the converged state and does not fail in the mid way.
...
% \begin{enumerate}[label=(\alph*)]
% \item The final state $\uu^{n+1} \in G$, which means that the solution to the nonlinear equation \eqref{eq:ESDIRK4} must be admissible.
% % \item All intermediate states $\uu^{(s,m)} \in G$, which means that the iterative solutions to the stage equation \eqref{eq:stage-equation} with each pseudo-time updating \eqref{eq:linearTauUpdate} must also be admissible,
% \item All intermediate states $\uu^{(s,m)} \in G$, which means that the iterative solution to the stage equation \eqref{eq:stage-equation} updated at each pseudo-time step, must also be admissible,
% \end{enumerate}
% where $\uu^{(s,m)}$ denotes the intermediate solution at the $m$-th inner iteration of stage $s$.
To ensure the admissibility of cell averages, the following conditions must be satisfied:
\begin{enumerate}[label=(\alph*)]
...
Guaranteeing the admissibility of the final state ensures that a physically meaningful converged solution exists. Ensuring the admissibility of all intermediate states is equally important, as it allows the iterative process to proceed successfully without encountering failures along the way. 
...
% In the ESDIRK4 time integration, the the future state is taken as the converged result of the last state. One may wonder that, it might be sufficient to ensure only the admissibility of the intermediate states. However, the inner iterations cannot be able to converge if the existence of an admissible converged state is not guaranteed. In other words, the iterative solution process of ESDIRK4 cannot converge if the nonlinear equation \eqref{eq:ESDIRK4} does not have physical solution. In this scenario, nonphysical (such as unbounded) solutions may be generated during the inner iteration and still cause simulation failure. Therefore, ensuring the admissibility of both the intermediate and the converged state is essential. 

% Without controlling the admissibility of the final state with physical-time step size, a positivity-preserving solution could still possibly be obtained by imposing positivity on the intermediate states while achieving convergence of the nonlinear residuals \eqref{eq:define-inc} and solving the equations successfully. 
% However, not controlling the final future state with physical-time step limiting would likely cause the situation where there is no admissible solution to the nonlinear equations \eqref{eq:ESDIRK4} and \eqref{eq:stage-equation}, and consequently convergence is impossible. 
% Given an ODE solver like ESDIRK4, since the implicit stage equations \eqref{eq:ESDIRK4} and \eqref{eq:stage-equation} are determined only by the RHS terms and the time step size, without explicitly modifying the RHS terms, physical-time step limiting is the sole mechanism that is able to control the existence of a converged future state.
...
% To address this challenge, we propose to estimate the solution change within one time step, based on a simple linear approximation. The estimated solution change is proportional to the time step size. 
% Admissible solution at the next time step can be guaranteed by limiting the time step size to ensure that the relative solution change is smaller than a certain threshold, which imposes a lower bound on the estimated future state. 
% The time step size is limited by imposing a lower bound on the estimated future state. 
% If the lower bound is larger than the magnitude of the estimation error, the future state is guaranteed to be positive.   
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=\linewidth]{pics/sketch_m.pdf}
    \caption{\added[id=r1]{Flowchart of the proposed positivity-preserving algorithm.}}
    \label{fig:sketch}
\end{figure}
...
In this work, a linear approximation is used to estimate the future state. At time $t^n$, given an admissible state $\uu^n$, the future state $\uu^{n+1}$ is estimated by
\begin{equation}
...
% Therefore, the parameter $\eta_t$ can be used to control the allowable solution change. A larger $\eta_t$ leads to a larger range for the relative solution change. The principle of selecting the value of parameter $\eta_t$ is discussed in Section \ref{ssec:influence-parameters}. The analysis in Section \ref{ssec:influence-parameters} shows that there exists an upper bound $\eta_t^c$ (close to 1) beyond which positivity is not guaranteed. While this bound is difficult to determine \emph{a priori}, we propose an \emph{a posteriori} strategy to iteratively select a parameter value within finite steps that can ensure admissible future state.
...
It is noted that, at each pseudo-time step, the updated intermediate solution is obtained using a single LU-SGS iteration. Consequently, this solution does not represent the accurate solution of the linear system described by \eqref{eq:linearTauUpdate}. As a result, the accuracy of the estimation in \eqref{eq:estimation-dtau} cannot be theoretically guaranteed, nor can the positivity of the updated solution. %$\UM^{n,s,m+1}_i$.
...
\label{eq:alpha-inc-p}
    ...
    \end{dcases}
\end{equation}
The updated solution $\uu^{\left(s,m+1\right)}_i \in G$ is then computed by
\begin{equation}
...
% As observed in Algorithm \ref{alg:pp-algorithm}, no additional vectors or matrices are allocated during its implementation, ensuring no extra memory cost. Furthermore, according to the computational cost analysis in Section \ref{ssec:sedov}, the positivity-preserving algorithm accounts for only 10\% of the total CPU time when the reconstruction polynomial scaling is activated on more than $20\%$ of the cells, demonstrating computational efficiency of the algorithm.
 ...
 \begin{algorithm}[htbp!]
 	...
 	\begin{algorithmic}[1] 
        ...
        \State Perform reconstruction using cell-averaged solution $\UM^{n}$ %at time step $n$ %$\left\{\UM_i^{n}\right\}_{i=1}^N$
        ...
        \State Perform reconstruction using cell-averaged solution $\UM^{\left(s,m\right)}$ %at time step $n$ %$\left\{\UM_i^{n}\right\}_{i=1}^N$
        % \State Perform reconstruction and shock limiter using solution $\UM^{\left(s,m\right)}$ %at time step $n$ %$\left\{\UM_i^{n}\right\}_{i=1}^N$

        \State Apply the WBAP limiter to reconstruction polynomials to suppress oscillations
        
        \State Apply the scaling limiter to the limited reconstruction polynomials according to \eqref{eq:polynomial-scaling}
        
        % \State Perform scaling limiter on the polynomials according to \eqref{eq:polynomial-scaling}
