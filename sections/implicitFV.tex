...
\end{equation}}{\begin{equation}
    ...
    % \left[\F \left(\U \left(\x_{f,g},t\right)\right) - \F_v \left(\U\left(\x_{f,g},t\right), \nabla \U \left(\x_{f,g},t\right)\right)\right] \cdot \n_f = \tilde{\F} \left(\U_L, \U_R,\n_f\right)- \tilde{\F}_v \left(\U_L, \nabla \U_L, \U_R, \nabla \U_R, \n_f\right),
    \left. \left(\F - \F_v \right)\right|_{\x=\x_{f,g}} \cdot \n_f = \tilde{\F} \left(\U_L, \U_R,\n_f\right)- \tilde{\F}_v \left(\U_L, \U_R, \nabla \U_L, \nabla \U_R, \n_f\right),
\end{equation}}where $\tilde{\F}$ and $\tilde{\F}_v$ are the inviscid and viscous flux schemes, respectively, with the left and right states at the quadrature point defined by
\begin{equation}
    \begin{dcases}
...
\begin{equation}
    ...
    \derivative{\uu_i}{t} = \R_i,
\end{equation}
which can be integrated in time to update the cell averages in a step-by-step manner. The time integration method used in this work will be presented in Section \ref{ssec:TimeMarching}.}
...
\added[id=r1]{
In general, the conservative variables are reconstructed separately, 
allowing an implementation of solution reconstruction 
in a variable-by-variable manner.
...
\added[id=r1]{
The conditioning of the global reconstruction matrix $\mathbf{A}$ in \eqref{eq:global-system} is numerically investigated using the four meshes shown in Figure \ref{fig:meshes_test_cond}, with periodic boundary conditions applied. Cubic variational reconstruction is performed on each mesh. Additionally, the spectral radius $\rho_{GS}$ of the iteration matrix from the block Gauss-Seidel scheme in \eqref{eq:gauss-seidel-iteration} is computed to assess the convergence behavior of the variational reconstruction.
The results for the condition number and spectral radius for each mesh are summarized in Table \ref{tab:cond_and_rho}. On regular meshes, both the condition number and the spectral radius are smaller, while on distorted meshes, both increase but remain within reasonable limits. In all cases, the spectral radius stays below $1$, consistent with the theoretical analysis of block Gauss-Seidel iteration for variational reconstruction \cite{wang2017compact_VR}.
}
% \added[id=r1]{
% The conditioning of the global reconstruction matrix $\mathbf{A}$ in \eqref{eq:global-system} is numerically investigated on the four meshes displayed in Figure \ref{fig:meshes_test_cond}. Periodic boundary conditions are applied. 
...
% Cubic variational reconstruction is performed on the meshes. 
% Meanwhile, the spectral radius of the iteration matrix arising from block Gauss-Seidel iteration in \eqref{eq:gauss-seidel-iteration}, $\rho_{GS}$, is calculated on these meshes to investigate the convergence behavior of variational reconstruction. 
% }

% \added[id=r1]{
% Results on condition number and spectral radius for meshes in Figure \ref{fig:meshes_test_cond} are listed in Table \ref{tab:cond_and_rho}. 
% It is shown that on regular meshes, both the condition number of the global reconstruction matrix and the spectral radius of the iteration matrix are smaller.
% On distorted meshes, the condition number and spectral radius are increased,
% but still remain in reasonable ranges. 
% The spectral radius results all stay below $1$,
% agreeing with the general theoretical
% analysis of block Gauss-Seidel iteration in varitaional reconstruction \cite{wang2017compact_VR}.
% }

\begin{table}[htbp!]
    \centering
    \caption{\added[id=r1]{Condition numbers and spectral radii on the four meshes shown in Figure \ref{fig:meshes_test_cond}.}}
...
%    \hfill
   ...
   \end{subfigure}
   \caption{\added[id=r1]{Meshes used for condition number and spectral radius tests of variational reconstruction.}}
...
% % {\color{r2color}The advantage of dual-time stepping over the approximate Newton is that the pseudo-time derivative may be chosen so as to optimize the convergence of the inner iterations \cite{venkateswaran1995dual}.}
...
{In this work, the time integration scheme for unsteady flow simulation is the six-stage, fourth-order explicit first stage singly diagonal implicit Runge-Kutta (ESDIRK4) \cite{bijl2002implicitBDFvESDIRK}. By integrating the ODE \eqref{eq:FVODECell} in time, the cell averages are updated by
\begin{subequations}\label{eq:ESDIRK4}
    \begin{align}
...
The pseudo-time derivative is discretized using a simple first-order finite difference as follows:
\begin{equation}
